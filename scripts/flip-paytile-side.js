...
async function flipSide() {
  ...
  console.log('Flipped Paytile two-column: image LEFT, text+label RIGHT (side: right).');
}

flipSide().catch(e => { console.error(e); process.exit(1); }).finally(async () => { await prisma.$disconnect(); await pool.end(); });