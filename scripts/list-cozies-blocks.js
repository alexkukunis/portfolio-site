...
async function main() {
  ...
  console.log(`Cozies slug: ${cozies.slug}, ID: ${cozies.id}, title: ${cozies.title}, role: ${cozies.role}`);
  const blocks = await prisma.caseStudyBlock.findMany({
    where: { caseStudyId: cozies.id },
    orderBy: { order: 'asc' },
    select: { id: true, type: true, content: true, order: true }
  });
  console.log('All Cozies blocks:', JSON.stringify(blocks, null, 2));
}
